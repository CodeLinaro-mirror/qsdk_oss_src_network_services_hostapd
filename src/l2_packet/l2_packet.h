...
enum l2_packet_filter_type {
	...
	L2_PACKET_FILTER_1905_MSG_TYPE,   /* IEEE 1905.1 EtherType + SMD msg_type */
};
...
/**
 * l2_packet_set_bpf_filter - Attach a caller-built BPF program to the socket
 * @l2: Pointer to internal l2_packet data from l2_packet_init()
 * @insns: Array of struct sock_filter BPF instructions (cast to void *)
 * @len: Number of instructions (not bytes)
 * Returns: 0 on success, -1 on failure
 *
 * Used by eth_p_1905.c to install a dynamically-constructed filter for
 * EtherType=0x893A + specific 1905 message type(s). The insns pointer
 * is cast to struct sock_filter * inside the Linux implementation.
 */
