			 ...
			 int pairwise_cipher, enum mfp_options mfp)
{
	...
	rsn_ie = get_ie(wpa_ie, wpa_ie_len, WLAN_EID_RSN);
	if (!rsn_ie || wpa_parse_wpa_ie_rsn(rsn_ie, 2 + rsn_ie[1], &data) < 0) {
		wpa_printf(MSG_ERROR, "No valid AP RSNE");
		return -1;
	}
