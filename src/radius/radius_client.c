...
struct radius_client_data {
	...
	int tls_reconnect_attempts;
	int tls_reconnect_max_attempts;
	int tls_reconnect_max_delay;
...
void radius_tls_reconnect(void *eloop_ctx, void *timeout_ctx)
{
	...
	wpa_printf(MSG_DEBUG, "RADIUS: TLS reconnect attempt %d",
		   radius->tls_reconnect_attempts);

	radius->tls_reconnect_pending = false;
	/* Until max reconnect attempts are reached, retry connection to
	 * the same server
	 */
...
{
	int delay_sec, jitter;

	delay_sec = 1 << radius->tls_reconnect_attempts;
	jitter = os_random() % 10000;
...
static int radius_client_init_auth(struct radius_client_data *radius)
{
	...
	if (radius->tls_reconnect_pending) {
		wpa_printf(MSG_DEBUG,
			   "RADIUS: TLS Reconnect pending, "
			   "Skip packet-triggered reconnect");
		return 0;
	}
#endif
	radius_close_auth_socket(radius);
