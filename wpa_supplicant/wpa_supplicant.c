...
/* Note: @ssid->bssid_set is expected to be set */
static bool wpas_get_ssid_match_bssid(struct wpa_supplicant *wpa_s,
				      struct wpa_ssid *ssid,
				     ...
				     const struct wpa_interface *iface)
{
	...
	if (wpa_s->hw.modes) {
		...
		for (i = 0; i < wpa_s->hw.num_modes; i++) {
			if (wpa_s->hw.modes[i].uhr_capab[IEEE80211_MODE_INFRA].
			    uhr_supported)
				wpa_s->hw_capab |= BIT(CAPAB_UHR);
